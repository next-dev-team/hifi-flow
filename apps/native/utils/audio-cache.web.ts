...
class AudioChunkCache {
  ...
  async markFileComplete(url: string, contentLength: number, contentType: string) {
      await this.initPromise;
      return new Promise<void>((resolve, reject) => {
          if (!this.db) return reject(new Error('DB not initialized'));
          const tx = this.db.transaction(['files'], 'readwrite');
          const store = tx.objectStore('files');
          const request = store.put({ url, contentLength, contentType, timestamp: Date.now() }, url);
          request.onsuccess = () => resolve();
          request.onerror = () => reject(request.error);
      });
  }

  async getFileInfo(url: string): Promise<{ contentLength: number, contentType: string } | undefined> {
      await this.initPromise;
      return new Promise((resolve, reject) => {
          if (!this.db) return reject(new Error('DB not initialized'));
...
export class ChunkedAudioLoader {
  ...
  constructor(url: string, chunkSize = 256 * 1024) { // 256KB chunks
    this.url = url;
  ...
  async getAudioUrl(): Promise<string> {
      ...
      if (cachedInfo) {
          ...
          try {
              const blob = await this.reconstructBlob(cachedInfo.contentLength, cachedInfo.contentType);
  ...
  private async reconstructBlob(totalSize: number, contentType: string): Promise<Blob> {
  ...
  async cacheFullAudio(): Promise<void> {
      ...
      await this.cache.markFileComplete(this.url, this.totalSize, this.contentType);
...
export const audioCacheService = {
  resolveUrl: async (url: string) => {
    ...
    if (resolved === url) {
        // Fire and forget
        loader.cacheFullAudio().catch(e => console.warn('[AudioLoader] Background cache failed', e));
    }
