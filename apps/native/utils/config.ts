...
function ensureWeightedTargets(
  ...
  if (apiVersion === "v2") {
    ...
    if (!v1WeightedTargets) {
      ...
      v1WeightedTargets = buildWeightedTargets([
        ...ALL_API_TARGETS,
        ...v2Fallback,
      ]);
    }
...
export function selectApiTarget(
  apiVersion: "v1" | "v2" = "v2"
): ApiClusterTarget {
...
function selectFromWeightedTargets(
...
export function getTargetsForRegion(
...
export function selectApiTargetForRegion(
...
export function getProxiedUrl(url: string): string {
  ...
  return `${API_CONFIG.proxyUrl}?url=${encodeURIComponent(
    targetUrl.toString()
  )}`;
}
...
function isLikelyProxyErrorEntry(entry: unknown): boolean {
  ...
  const subStatus =
    typeof record.subStatus === "number" ? record.subStatus : undefined;
  const userMessage =
...
export async function fetchWithCORS(
  ...
  for (let attempt = 0; attempt < totalAttempts; attempt += 1) {
    ...
    const rewrittenPath = combinePaths(
      targetBase.pathname || "/",
      relativePath
    );
