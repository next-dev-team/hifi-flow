 ...
 */
export function formatArtistsForMetadata(
...
type RegionOption = Parameters<typeof selectApiTargetForRegion>[0];
...
class LosslessAPI {
  ...
  private resolveRegionalBase(region: RegionOption = "auto"): string {
    try {
      ...
      console.warn(
        "Falling back to default API base URL for region selection",
        { region, error }
      );
    }
    return this.baseUrl;
  ...
  private buildSearchResponse<T>(
    ...
    const limit =
      typeof section?.limit === "number" ? section.limit : list.length;
  ...
  private parseTrackLookup(data: unknown): TrackLookup {
    ...
    for (const entry of entries) {
      ...
      if (!originalTrackUrl && "OriginalTrackUrl" in entry) {
        const candidate = (entry as { OriginalTrackUrl?: unknown })
          .OriginalTrackUrl;
  ...
  private extractStreamUrlFromManifest(manifest: string): string | null {
    try {
      ...
      try {
        ...
        console.debug(
          "Manifest JSON parse failed, falling back to pattern match",
          jsonError
        );
      }
      ...
      let match = urlRegex.exec(decoded);
      while (match !== null) {
        const url = match[0];
  ...
  private isXmlContentType(contentType: string | null): boolean {
    ...
    return (
      ...
      /dash\+xml|mpd/i.test(contentType)
    );
  }
  ...
  private isJsonContentType(contentType: string | null): boolean {
    ...
    return (
      ...
      /application\/vnd\.tidal\.bts/i.test(contentType)
    );
  }
  ...
  private isV2ApiContainer(
    payload: unknown
  ): payload is { version?: unknown; data?: unknown } {
    return Boolean(
      payload &&
  ...
  private buildTrackInfoFromV2(
    data: Record<string, unknown>,
    fallbackTrackId: number
  ): TrackInfo {
    const manifestMimeType =
      typeof data.manifestMimeType === "string" &&
      data.manifestMimeType.trim().length > 0
        ? data.manifestMimeType
        : "application/dash+xml";

    return {
      trackId:
        typeof data.trackId === "number" ? data.trackId : fallbackTrackId,
      audioMode: typeof data.audioMode === "string" ? data.audioMode : "STEREO",
      audioQuality:
        typeof data.audioQuality === "string" ? data.audioQuality : "LOSSLESS",
      manifest: typeof data.manifest === "string" ? data.manifest : "",
      manifestMimeType,
      manifestHash:
        typeof data.manifestHash === "string" ? data.manifestHash : undefined,
      assetPresentation:
        typeof data.assetPresentation === "string"
          ? data.assetPresentation
          : "FULL",
      albumReplayGain:
        typeof data.albumReplayGain === "number"
          ? data.albumReplayGain
          : undefined,
      albumPeakAmplitude:
        typeof data.albumPeakAmplitude === "number"
          ? data.albumPeakAmplitude
          : undefined,
      trackReplayGain:
        typeof data.trackReplayGain === "number"
          ? data.trackReplayGain
          : undefined,
      trackPeakAmplitude:
        typeof data.trackPeakAmplitude === "number"
          ? data.trackPeakAmplitude
          : undefined,
  ...
  private extractOriginalTrackUrl(
  ...
  private buildDashManifestResult(
    payload: string,
    contentType: string | null
  ): DashManifestResult {
    ...
    if (
      ...
      const parsed = this.parseJsonSafely<{ detail?: unknown; urls?: unknown }>(
        manifestText
      );
      if (
        parsed &&
        ...
        throw this.createDashUnavailableError(
          "Dash manifest not found for track"
        );
      }
    ...
    throw this.createDashUnavailableError(
      "Received unexpected payload from dash endpoint."
    );
  }
  ...
  private parseMpdSegmentTemplate(manifestText: string): {
    ...
    const parseWithDom = () => {
      ...
      try {
        ...
        const baseUrl =
          rawBaseUrl && this.isValidMediaUrl(rawBaseUrl)
            ? rawBaseUrl
            : undefined;
        ...
        const initializationUrl = template
        ...
        const timelineParent =
          template.getElementsByTagName("SegmentTimeline")[0];
    ...
    const parseWithRegex = () => {
      ...
      while (match !== null) {
        ...
        match = timelineRegex.exec(trimmed);
      }
  ...
  private buildMpdSegmentUrls(
    ...
    const timeline =
      template.segmentTimeline.length > 0
    ...
    for (const entry of timeline) {
      ...
      for (let i = 0; i < count; i += 1) {
        const url = template.mediaUrlTemplate.replace(
          "$Number$",
          `${segmentNumber}`
        );
  ...
  private async downloadFlacFromMpd(
    ...
    for (const url of urls) {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch DASH segment (status ${response.status})`
        );
      }
    ...
    return {
      blob: new Blob([merged], { type: "audio/flac" }),
      mimeType: "audio/flac",
    };
  }
  ...
  async getTrack(
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      const message =
        detail ?? `Failed to get track (status ${response.status})`;
  ...
  async getDashManifestWithMetadata(
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        ...
        const result = this.buildDashManifestResult(
          manifestPayload,
          contentType
        );
        const trackInfo = {
    ...
    throw (
      lastError ??
      this.createDashUnavailableError("Unable to load dash manifest for track")
    );
  }
  ...
  async getArtist(id: number): Promise<ArtistDetails> {
    ...
    const isArtistLike = (value: unknown): value is Artist => {
      ...
      return (
        ...
        ("artistRoles" in candidate ||
          "artistTypes" in candidate ||
          "url" in candidate)
      );
    };
    ...
    const albumById = new Map(
      albums.map((album) => [album.id, album] as const)
    );
  ...
  async getStreamData(
    ...
    if (this.isHiResQuality(quality)) {
      try {
        ...
        console.warn(
          "Failed to resolve hi-res stream via DASH manifest",
          error
        );
        quality = "LOSSLESS";
      }
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        ...
        const manifestUrl = this.extractStreamUrlFromManifest(
  ...
  private buildMetadataEntries(lookup: TrackLookup): Array<[string, string]> {
    ...
    entries.push([
      "comment",
      "Downloaded from music.binimum.org/tidal.squid.wtf",
    ]);
  ...
  private async runMetadataEmbedding(
    ...
    const shouldConvertToMp3 =
      convertToMp3 && convertibleExtensions.has(extension);
    ...
    if (options?.onFfmpegCountdown) {
      try {
        const estimatedBytes =
          await ffmpegModule.estimateFfmpegDownloadSize?.();
    ...
    try {
      ...
      if (artworkId) {
        ...
        const coverSizes: Array<"1280" | "640" | "320"> = [
          "1280",
          "640",
          "320",
        ];
      ...
      if (outputData instanceof Uint8Array) {
        ...
        outputArray = new Uint8Array(
          (outputData as unknown as ArrayBuffer) ?? new ArrayBuffer(0)
        );
      }
      ...
      if (
        ...
        options?.onFfmpegError?.(
          new Error(
            "Memory error: File processed without metadata due to browser limitations"
          )
        );
      } else {
        options?.onFfmpegError?.(error);
  ...
  async fetchTrackBlob(
    ...
    try {
      ...
      if (streamUrl) {
        ...
        if (!response.ok) {
          console.warn(
            "OriginalTrackUrl download failed, falling back to manifest",
            {
      ...
      if (!response) {
        ...
        const decodedManifest = this.decodeBase64Manifest(
          manifestSource.info.manifest
        );

        // For segmented DASH manifests, go directly to segment download
        if (this.isSegmentedDashManifest(decodedManifest)) {
          try {
            const mpdResult = await this.downloadFlacFromMpd(
              decodedManifest,
              options
            );
            if (mpdResult) {
          ...
          let fallbackUrl = this.extractStreamUrlFromManifest(
            manifestSource.info.manifest
          );
          if (!fallbackUrl && manifestQuality !== "LOSSLESS") {
            try {
              const losslessLookup = await this.getTrack(trackId, "LOSSLESS");
              const candidateUrl = this.extractStreamUrlFromManifest(
                losslessLookup.info.manifest
              );
              if (candidateUrl) {
      ...
      if (
        ...
        error.message === RATE_LIMIT_ERROR_MESSAGE
      ) {
        throw error;
      }
      throw new Error(
  ...
  async downloadTrack(
    ...
    try {
      const { blob } = await this.fetchTrackBlob(
        trackId,
        quality,
        filename,
        options
      ...
      if (options?.downloadCoverSeperately) {
        try {
          const metadata = await this.getPreferredTrackMetadata(
            trackId,
            quality
          );
          ...
          if (coverId) {
            console.log(
              "[Cover Download] Fetching cover for separate download..."
            );
            ...
            for (const size of coverSizes) {
              ...
              console.log(
                `[Cover Download] Attempting size ${size}:`,
                coverUrl
              );
              ...
              for (const strategy of fetchStrategies) {
                ...
                console.log(
                  `[Cover Download] Trying strategy: ${strategy.name}`
                );
                ...
                try {
                  ...
                  console.log(
                    `[Cover Download] Response status: ${
                      coverResponse.status
                    }, Content-Length: ${coverResponse.headers.get(
                      "Content-Length"
                    )}`
                  );
                  ...
                  if (contentLength && parseInt(contentLength, 10) === 0) {
                    console.warn(
                      `[Cover Download] Content-Length is 0 for size ${size}`
                    );
                    continue;
                  }
                  ...
                  if (contentType && !contentType.startsWith("image/")) {
                    console.warn(
                      `[Cover Download] Invalid content type: ${contentType}`
                    );
                    continue;
                  }
                  ...
                  if (!arrayBuffer || arrayBuffer.byteLength === 0) {
                    console.warn(
                      `[Cover Download] Empty array buffer for size ${size}`
                    );
                    continue;
                  }
                  ...
                  const uint8Array = new Uint8Array(arrayBuffer);
                  console.log(
                    `[Cover Download] Received ${uint8Array.length} bytes`
                  );
                  console.log(
                  ...
                  if (!this.validateImageData(uint8Array)) {
                    console.warn(
                      `[Cover Download] Invalid image data for size ${size}`
                    );
                    continue;
                  }
                  ...
                  if (!imageFormat) {
                    console.warn(
                      `[Cover Download] Unknown image format for size ${size}`
                    );
                    continue;
                  }

                  // Create blob with correct MIME type
                  const coverBlob = new Blob([uint8Array], {
  ...
  getVideoCoverUrl(
    ...
    return `https://resources.tidal.com/videos/${videoCoverId.replace(
      /-/g,
      "/"
    )}/${size}x${size}.mp4`;
  }
  ...
  getArtistPictureUrl(pictureId: string, size: "750" = "750"): string {
    return `https://resources.tidal.com/images/${pictureId.replace(
      /-/g,
      "/"
    )}/${size}x${size}.jpg`;
  }
